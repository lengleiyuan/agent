package cd.lan1akea.core.model.custom;
 ...
 * 自定义聊天模型实现。
 * <p>
 * 对接第三方 OpenAI 兼容 API，支持自定义 endpoint 和认证方式。
 * </p>
 */
public class CustomChatModel extends ChatModelBase {
    ...
    public CustomChatModel(String providerName, String apiKey, String modelName, String baseUrl) {
        this(providerName, apiKey, modelName, baseUrl, "Authorization", "Bearer ",
            new OpenAiMessageFormatter());
    }

    public CustomChatModel(String providerName, String apiKey, String modelName,
                            String baseUrl, String authHeaderName, String authHeaderPrefix,
                            MessageFormatter formatter) {
        super(providerName, modelName, formatter);
        this.providerName = providerName;
        this.apiKey = apiKey;
        this.baseUrl = baseUrl;
        this.authHeaderName = authHeaderName;
        this.authHeaderPrefix = authHeaderPrefix != null ? authHeaderPrefix : "";
    ...
    protected Mono<ChatResponse> doChat(List<Map<String, Object>> formattedMessages,
        ...
        return Mono.fromCallable(() -> buildRequestBody(formattedMessages, toolSchemas, options, false))
            .flatMap(body -> httpClient.post(baseUrl + "/chat/completions", buildHeaders(), body))
    ...
    protected Flux<ChatStreamChunk> doStream(List<Map<String, Object>> formattedMessages,
        ...
        return Mono.fromCallable(() -> buildRequestBody(formattedMessages, toolSchemas, options, true))
            .flatMapMany(body -> httpClient.postStream(baseUrl + "/chat/completions", buildHeaders(), body))
            .transform(sseParser::parse);
    }

    private Map<String, String> buildHeaders() {
        Map<String, String> headers = new LinkedHashMap<>();
        headers.put(authHeaderName, authHeaderPrefix + apiKey);
        return headers;
    }
